  ...
  return (
    <div className='coin-search__container'>
      <input
        ...
        onBlur={(e) => e.relatedTarget?.closest('.coin-search__item') || setInputFocus(false)}
      />
      {dropdown && <ul className='coin-search__drop-down'>
        { isFetching && <div>Loading...</div> }
        { data?.map(coin => (
          <li className='coin-search__item' key={coin.id}>
            <Link onClick={() => setSearch('')} to={`${ROUTES.COINS}/${coin.id}`}>
              <img src={coin.thumb} alt="c" />
              <span>{`${coin.name} (${coin.symbol.toUpperCase()})`}</span>
              {coin.market_cap_rank && <span className='coin-search__rank'>{`#${coin.market_cap_rank}`}</span>}
            </Link>
          </li>
        )) }
      </ul>}

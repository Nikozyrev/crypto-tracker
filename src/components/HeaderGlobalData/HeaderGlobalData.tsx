...
export const HeaderGlobalData = () => {
  ...
  return (
    <div className='header__global-data'>
      {data && 
      <>
        <div><span>Coins: </span><span>{data?.active_cryptocurrencies}</span></div>
        <div><span>Exchanges: </span><span>{data?.markets}</span></div>
        <div><span>Market Cap: </span><span>{priceFormatter(currency)(data?.total_market_cap[currency])}</span></div>
        <div><span>24h Vol: </span><span>{priceFormatter(currency)(data?.total_volume[currency])}</span></div>
        <div>
          <span>Dominance: </span>
